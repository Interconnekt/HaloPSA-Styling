...
**Shared typography** for every `.status-avatar.{fortable,small,bitsmall,smaller,smallest,smallester,smallestest,fortile}`: Figtree / 10.5px / weight 600 / 0.06em tracking / uppercase. Exception: the redundant KB-article Custom-CSS override and the ticket-list table header both intentionally use the DISPLAY font, see §1's cascade note.
